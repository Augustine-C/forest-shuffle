...
exports.executeBonus = executeBonus;
exports.drawCardsOneByOne = drawCardsOneByOne;
const cards_1 = require("./cards");
const cardMatching_1 = require("./cardMatching");
/**
 * Execute a card's effect
 */
function executeEffect(context) {
    const effectText = (0, cards_1.getCardEffect)(context.card, context.speciesIndex);
    const result = {
        cardsDrawn: 0,
        cardsMoved: [],
        extraTurn: false,
    ...
    if (drawPerMatch) {
        ...
        if (target.includes('Deer')) {
            count = (0, cardMatching_1.countCardsWithTag)(context.player.forest, 'Deer');
        }
 ...
 */
function executeBonus(context) {
    const bonusText = (0, cards_1.getCardBonus)(context.card, context.speciesIndex);
    const result = {
        cardsDrawn: 0,
        cardsMoved: [],
        extraTurn: false,
 ...
 */
function drawCardsOneByOne(gameState, player, count) {
    const drawn = [];
