...
const effectPatterns = new Set<string>();
const bonusPatterns = new Set<string>();
const pointsPatterns = new Set<string>();

Object.values(SPECIES_DATA).forEach(species => {
    if (species.effect) effectPatterns.add(species.effect);
    if (species.bonus) bonusPatterns.add(species.bonus);
    if (species.points) pointsPatterns.add(species.points);
});
...
const drawEffects = Array.from(effectPatterns).filter(e =>
    e.includes('Receive') || e.includes('card')
);
console.log(`\n  Draw Cards (${drawEffects.length}):`);
drawEffects.slice(0, 3).forEach(e => console.log(`    - ${e}`));

const playFreeEffects = Array.from(effectPatterns).filter(e =>
    e.includes('Play') && e.includes('free')
);
console.log(`\n  Play Free Cards (${playFreeEffects.length}):`);
playFreeEffects.slice(0, 3).forEach(e => console.log(`    - ${e}`));

const caveEffects = Array.from(effectPatterns).filter(e =>
    e.includes('cave')
);
console.log(`\n  Cave Effects (${caveEffects.length}):`);
caveEffects.slice(0, 3).forEach(e => console.log(`    - ${e}`));

const turnEffects = Array.from(effectPatterns).filter(e =>
    e.includes('another turn')
);
...
const fixedPoints = Array.from(pointsPatterns).filter(p =>
    p.match(/^Gain \d+ points?$/)
);
console.log(`\n  Fixed Points (${fixedPoints.length}):`);
fixedPoints.slice(0, 3).forEach(p => console.log(`    - ${p}`));

const perCardPoints = Array.from(pointsPatterns).filter(p =>
    p.includes('for each') || p.includes('per')
);
console.log(`\n  Per Card/Symbol (${perCardPoints.length}):`);
perCardPoints.slice(0, 3).forEach(p => console.log(`    - ${p}`));

const conditionalPoints = Array.from(pointsPatterns).filter(p =>
    p.includes('if ') || p.includes('at least')
);
console.log(`\n  Conditional (${conditionalPoints.length}):`);
conditionalPoints.slice(0, 3).forEach(p => console.log(`    - ${p}`));

const accordingToPoints = Array.from(pointsPatterns).filter(p =>
